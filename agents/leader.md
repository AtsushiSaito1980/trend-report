...
### 0.5. 直近トピックのチェック（重複回避）
1. `reports/index.json` を読み込む
2. 今日から7日以内（`date` フィールドで判定）のエントリの `topics` 配列をすべて抽出
3. それらを「除外リスト」としてメモする

### 0.6. トピック選定ディスカッション（トークン節約のため4行以内の回答のみ受け付ける）
1. `topics.json` の全候補から除外リストを引いた「候補リスト」を作成
2. Tech / Human / Critic の各エージェントに以下を依頼する：
   > 「候補リスト: {A, B, C, D, E, …}  あなたの視点で最も面白い2件を選び、4行以内で回答してください」
3. 各エージェントの返答形式（これ以外の回答は受け付けない）：
   ```
   [Role] 候補X, 候補Y
   候補X: (1行理由)
   候補Y: (1行理由)
   ```
4. 票を集計し、最多推薦の **3件** を最終選定トピックとする（同点はリーダーが独断）

### 1. スカウト（WebSearch を自分で実行）
1. `topics.json` を読み込む（選定済みトピックを使用）
2. **0.6で選んだ3トピック**を使う（ランダム選択ではなく討議結果を優先）
