...
[リーダー＝スカウト]
  ...
  1. Tech/Human/Critic に「候補トピック一覧」を提示し、各自が推薦2件+1行理由を返す（4行以内）
  2. 最多推薦かつ直近7日と被らない3トピックを選定
...
## トピック選定ディスカッション（直近7日と重複しないトピックを選ぶ）
- リーダーが reports/index.json の直近7日のエントリから `topics` フィールドを抽出し「除外リスト」を作成
- topics.json の全候補から除外リストを引いた「候補リスト」を Tech/Human/Critic に提示
- 各エージェントは **4行以内** で回答（トークン節約のため厳守）：
  ```
  [Role] 候補X, 候補Y
  候補X: (1行理由)
  候補Y: (1行理由)
  ```
- リーダーが票を集計し、最多推薦の3件を選定（同点はリーダーが独断決定）
